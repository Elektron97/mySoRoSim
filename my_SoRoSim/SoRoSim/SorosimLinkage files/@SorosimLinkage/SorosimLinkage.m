...
classdef SorosimLinkage < matlab.System
    properties% Independent properties of Linkage class
        ...
        q_scale      %(ndofx1) array of multiplier for each joint coordinate avoid
        ...
        CablePoints       %Struct with cell elements (Cy_fn and Cz_fn) of parameterized functions corresponding to the y and z coodinates of the cable
    ...
    methods
        ...
        function Tr = SorosimLinkage(varargin)
            ...
            Tr.VLinks = VLinks;
            Tr.N = 0;
            Tr.PlotParameters.Lscale = VLinks(1).L;
            Tr.ndof = 0;
            ...
            for i=1:N
                ...
                ndof         = Tr.ndof;
                VTwists_i    = Twist.empty(Tr.VLinks(LinkIndex_i).npie,0);
                ...
                while ~happy
                    ...
                    for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %for each of the soft link divisions\
                        ...
                        T               = Twist(i,j,Tr.VLinks(Tr.LinkIndex(i)).Xs{j}); %calling Twist class to generate twist for each piece of soft link
                        VTwists_i(j+1)  = T;
            ...
            if nCLj>0
                ...
                Tr.VTwistsCLj = Twist.empty(nCLj,0);
                ...
                for iCL=1:nCLj
                    ...
                    for i=1:iA
                        ...
                        for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1
                            i_sigAiCL = i_sigAiCL+Tr.VLinks(Tr.LinkIndex(i)).nGauss{j};
                        end
                    end
                    ...
                    for i=1:iB
                        ...
                        for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1
                            i_sigBiCL = i_sigBiCL+Tr.VLinks(Tr.LinkIndex(i)).nGauss{j};
                        end
                    end
                ...
                Tr.CLprecompute.i_sigA = i_sigA; %sig point of tip of A
                Tr.CLprecompute.i_sigB = i_sigB; %sig point of tip of B
                Tr.T_BS                = 0.1; %desired settling time constant
                    ...
                    for ii=1:np
                        ...
                        while ~vrgood
                            prompt           = {'Link number (choose from figure) corresponding to the point of application of force/moment '...
                                ,'Division number (keep 1 for Rigid Link)'...
                            ...
                            definput         = {num2str(Tr.N),'1','0','0','0','0','1','0'};
                            opts.Interpreter = 'tex';
                        ...
                        Tr.Fp_loc{ii} = [linknum divnum];

                        syms t;
                        Mxs = str2sym(Fp_ans{3});
                        Mys = str2sym(Fp_ans{4});
                        Mzs = str2sym(Fp_ans{5});
                        Fxs = str2sym(Fp_ans{6});
                        Fys = str2sym(Fp_ans{7});
                        Fzs = str2sym(Fp_ans{8});
            ...
            switch Answer
                ...
                case 'Yes'
                    ...
                    [n_jact,i_jact,i_jactq,WrenchControlled,Bqj1] = JointActuation(Tr);
                    ...
                    if any(Tr.linktype=='s')
                        ...
                        switch Answer2
                            ...
                            case 'Yes'
                                ...
                                [n_sact,dc,dcp,Sdiv,Ediv,Inside] = CableActuation(Tr);
                                ...
                                CablePoints.Cy_fn=Cy_fn;
                                CablePoints.Cz_fn=Cz_fn;
                                Tr.CablePoints = CablePoints;
        ...
        g       = FwdKinematics(Tr,q);                 %to get the transformation matrix at every significant points (arranged as column array)
        J       = Jacobian(Tr,q);                      %to get the Jacobian at every significant points (arranged as column array)
        Jd      = Jacobiandot(Tr,q,qd);                %to get the derivative of Jacobian at every significant points (arranged as column array)
        xi      = ScrewStrain(Tr,q)
        eta     = ScrewVelocity(Tr,q,qd);              %to get the screw velocity at every significant points (arranged as column array)
        D       = findD(Tr);                           %to compute and get the generalized damping matrix
        K       = findK(Tr)                            %to compute and get the generalized stiffness matrix
        Bq      = ActuationMatrix(Tr,q);               %to get the generalized actuation matrix (custom actuation not included)
        M       = GeneralizedMassMatrix(Tr,q)          %to get the generalized mass matrix
        C       = GeneralizedCoriolisMatrix(Tr,q,qd)   %to get the generalized coriolis matrix
        F       = GeneralizedExternalForce(Tr,q)       %to get the generalized external force matrix
        [t,qqd] = dynamics(Tr,odetype,dt);             %for dynamic simulation
        [q,u]   = statics(Tr,qu0,magnifier)            %for static simulation

        plotq0(Tr,Lh,Dh,CLh);       %to plot the free body diagram of the linkage
        plotq(Tr,q);                %to plot the state of the linkage for a given q
    ...
    methods % Property Update functions
        ...
        function Tr = Update(Tr)
            ...
            Tr.ndof    = size(Tr.K,1);
            Tr.nsig    = findnsig(Tr);
            [n_jact_new,i_jact_new,i_jactq_new,WrenchControlled_new,Bqj1_new] = JointActuation(Tr);

            Tr.n_jact            = n_jact_new;
            Tr.i_jact            = i_jact_new;
            Tr.i_jactq           = i_jactq_new;
            Tr.WrenchControlled  = WrenchControlled_new;
            Tr.Bqj1              = Bqj1_new;
        end
